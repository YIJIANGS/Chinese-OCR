...
clear;clc;close;
fdir=dir('字体100'); %读取文件路径
%%
for in=3:length(fdir)
[im,map]=imread(['字体100/',fdir(in).name]);
% im=ind2rgb(im,map);
grayim=double(rgb2gray(im))/255;
bw=grayim<.5;%二值化 阈值为0.5
% imshow(im)
%%
[row,col]=find(bw);
hrow=histcounts(row,max(row)-min(row)+1); %通过
hrow=[0,hrow>0];
...
%         rectangle('Position',[k,indx(i)+min(row)-2,wd_len,wd_high],'EdgeColor','r')
        hanzi_bw{wd_id}=grayim(indx(i)+min(row)-2:indx(i+1)+min(row),k:k+wd_len);%储存汉字图像到cell
        hanzi_bw{wd_id}=abs(1-imresize(hanzi_bw{wd_id},[30 30])); %图像统一尺寸
        wd_id=wd_id+1;
        k=k+wd_len; %更新汉字初始位置
        end
    end
